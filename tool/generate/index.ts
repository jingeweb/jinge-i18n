...
import { TranslateDictDir, TranslateMetaDir, CWD, HtmlInlineTags, InlineTags } from '../util';
...
let inited = false;
function initAlias(inlineTags: InlineTags) {
  if (inited) return;
  inited = true;
  const alias: Record<string, Record<string, string>> = {};
  Object.keys(inlineTags).forEach((k) => {
    const it = inlineTags[k];
    if (it.type === 'html') return;
    if (!(it.library in alias)) alias[it.library] = {};
    alias[it.library][it.component] = k;
  });
  TemplateParser.aliasManager.initialize(alias);
}
export function generate({ inlineTags }: { inlineTags: InlineTags }) {
  initAlias(inlineTags);
  const dict = loadDict();
  ...
  for (const file of dict.files) {
    ...
    switch (path.extname(file)) {
      ...
      case '.js':
        ...
        break;
      case '.ts':
        handleTs(source, file, dict, meta);
      default:
        break;
    }
  }
  ...
  meta.textRegisterMap.forEach((info, originalText) => {
    ...
    d?.compoents && imps.push(`${d.type || 'T'}Component`);
    d?.funcs && imps.push('t');
